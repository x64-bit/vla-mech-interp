...
Use `scripts/extract_ffn_vectors.py` to reproduce the Appendix A activation workflow. Record observations (dicts consumable by `policy.infer`) into `.npz` files, then run:

```bash
uv run python scripts/extract_ffn_vectors.py \
  --config-name pi0_droid \
  --checkpoint-dir /path/to/checkpoint \
  --observations-glob "/tmp/records/*.npz" \
  --output-path probes/pi0_droid.json \
  --top-k-per-layer 20 \
  --max-batches 128
```

The script hooks the PyTorch π₀ FFNs, replays your batches, and writes a JSON file listing the most active neurons per layer. Each entry contains the value vector (optional) and the top tokenizer logits so you can cluster or steer them offline.

To group semantic directions and identify “up/slow/etc.” clusters, run:

```bash
uv run python scripts/cluster_ffn_vectors.py \
  --projections-path probes/pi0_droid.json \
  --output-path probes/pi0_droid_clusters.json \
  --num-clusters 50 \
  --keywords up high
```

Finally, steer the policy at inference time by pointing to a steering config derived from those clusters:

```bash
uv run python scripts/apply_activation_steering.py \
  --config-name pi0_droid \
  --checkpoint-dir /path/to/checkpoint \
  --steering-config probes/up_cluster.json \
  --observations-glob "/tmp/records/*.npz"
```

The CLI reports per-batch action statistics for the baseline and steered runs so you can confirm the intervention before deploying it inside a robot loop.
